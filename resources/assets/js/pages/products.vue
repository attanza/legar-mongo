    ...
    <v-card class="pt-3">
      ...
      >
        <template slot="items" slot-scope="props">
          <td>{{ props.item.jenis_produk }}</td>
          <td>{{ props.item.nama_produk }}</td>
          <td>{{ props.item.ukuran }}</td>
          <td>{{ props.item.harga_dasar }}</td>
          <td>{{ props.item.harga_distributor }}</td>
          <td>{{ props.item.harga_reseller }}</td>
...
import { PRODUCTS_URL } from "../utils/apis.js";
...
export default {
  ...
  data: () => ({
    title: "Produk",
    headers: [
      { text: "Jenis Produk", align: "left", value: "jenis_produk" },
      { text: "Nama Produk", align: "left", value: "nama_produk" },
      { text: "Ukuran", align: "left", value: "ukuran" },
      { text: "Harga Dasar", align: "left", value: "harga_dasar" },
      { text: "Harga Distributor", align: "left", value: "harga_distributor" },
      { text: "Harga Reseller", align: "left", value: "harga_reseller" },
  ...
  methods: {
    async pupulateTable() {
      try {
        ...
        const endPoint = `${PRODUCTS_URL}?${this.getQueryParams()}`;
    ...
    toDetail(data) {
      window.location.href = `/manage/products/${data._id}`;
    },
    addData(data) {
